...
function menuInit() {
  let iconMenu = document.querySelector(".icon-menu--main");
...
function clickEvents(e) {
  if (e.target.closest(".header__city-title")) {
    ...
    if (cityBtn) {
      ...
      slideToggle(cityBtn.nextElementSibling);
    }
  }
  if (!document.querySelector(".header__city").contains(e.target)) {
    ...
    if (cityBtn) {
      ...
      slideUp(cityBtn.nextElementSibling);
    }
  }
}
//!======================================================================================================
let navArr = document.querySelectorAll(".menu__item");
if (navArr) {
  navArr.forEach((element) => {
    element.addEventListener("click", (e) => {
      const currentLi = e.target.closest(".menu__item");
      const wasActive = currentLi.classList.contains("_active");
      navArr.forEach((li) => li.classList.remove("_active"));
      if (!wasActive) {
        currentLi.classList.add("_active");
      }
    });
  });
  document.addEventListener("click", (event) => {
    const isClickInside = Array.from(navArr).some(
  ...
  if (data) {
    ...
    if (adressLink) {
      adressLink.forEach((e) => {
        e.textContent = data.adress;
      });
    }
    const footerCity = document.querySelector(".footer__city-title");
    if (footerCity) {
      footerCity.textContent = selectedCity;
    }
  }
}
document.addEventListener("selectCallback", function(event) {
  ...
  updateCityData(select);
});
window.addEventListener("load", function() {
  const select = document.querySelector('select[data-fls-select-id="1"]');
  if (select) {
    const savedCity = localStorage.getItem("selectedCity");
    if (savedCity && select.querySelector(`option[value="${savedCity}"]`)) {
